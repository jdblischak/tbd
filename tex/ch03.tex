...
treatment requires months of antibiotic therapy \citep{Sotgiu2015}, and
...
set), \mytilde11\% (7 of 65) of the individuals were classified
...
susceptibility status, our model predicted that \mytilde11\% of
...
We used limma+voom \citep{Smyth2004, Law2014, Ritchie2015} to implement
...
(\mytilde10\%). Because the two studies are substantially
...
We automated our analysis using Python (\url{https://www.python.org/}) and
...
\textbf{Batch processing.} We designed the processing of the samples to
...
\textbf{Principal components analysis (PCA) to identify outliers.} PC1 versus
...
\textbf{Comparing gene expression between the two studies.} After normalization
...
study and those in Barreiro et al., 2012 \citep{Barreiro2012}. The
...
\begin{table}[h]
\caption[Sample information.]{\textbf{Sample information.}
(see supplementary file associated with this dissertation)
Contains information on the 50 samples. Most
...
\ref{fig:outliers}) and removed from the analysis. (txt)
}
\label{ch03-s1}
\end{table}

\begin{table}[h]
\caption[Gene expression matrix.]{
\textbf{Gene expression matrix.}
(see supplementary file associated with this dissertation)
Contains the gene expression counts for the
...
``noninf'' for non-infected or ``infect'' for infected. (txt)
}
\label{ch03-s2}
\end{table}
...
\textbf{Differential expression results.}
(see supplementary file associated with this dissertation)
Contains the results of the differential
...
\begin{table}[h]
\caption[Data for combined analysis of gene expression data and GWAS results.]{
\textbf{Data for combined analysis of gene expression data and GWAS results.}
(see supplementary file associated with this dissertation)
Contains the results of the GWAS comparison
...
described for Supplementary Table \ref{ch03-s3} and contain the absolute log fold
changes for each comparison. All the other gene annotation columns are
the same as described for Supplementary Table \ref{ch03-s3}. The second sheet
...
(see supplementary file associated with this dissertation)
Contains the results of the classifier analysis.
...
the section for Supplementary Table \ref{ch03-s3}). The sheet ``training-input''
...
samples from Barreiro et al., 2012 \citep{Barreiro2012}. The column
